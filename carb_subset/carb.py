...
class Benchmark:
    ...
    def compare(self, predicted, matchingFunc, output_fn, error_file=None,
        ...
        for sent, goldExtractions in gold.items():
            ...
            scores = [[None for _ in predictedExtractions] for __ in
                      goldExtractions]
            ...
            sent_confidences = [extraction.confidence for extraction in
                                predictedExtractions]
            ...
            for conf in sent_confidences:
                ...
                if strategy == 'ss':
                    ...
                    for t in range(num_recall_matches):
                        ...
                        if matched_col == -1 or matched_row == -1:
                            raise Exception(
                                "error in CaRB, matched row/col is -1")
        ...
        try:
            ...
            zero_conf_point = (
            np.round(prec_scores[0], 4), np.round(rec_scores[0], 4),
            np.round(f1s[0], 4), confidence_thresholds[0])
        except ValueError:
    ...
    PREPS = ['above', 'across', 'against', 'along', 'among', 'around', 'at',
             'before', 'behind', 'below', 'beneath', 'beside', 'between', 'by',
             'for', 'from', 'in', 'into', 'near', 'of', 'off', 'on', 'to',
...
def f_beta(precision, recall, beta=1):
    ...
    return (1 + pow(beta, 2)) * (precision * recall) / (
...
if __name__ == '__main__':
    ...
    logging.info(
        "Writing PR curve of {} to {}".format(predicted.name, out_filename))
