...
class Matcher:
    ...
    def bowMatch(ref, ex, ignoreStopwords, ignoreCase):
        """
        A binary function testing for exact lexical match
        (ignoring ordering) between reference
    ...
    def bleuMatch(ref, ex, ignoreStopwords, ignoreCase):
        ref_arg1, ref_pred, ref_arg2 = ref.args[
            0].strip(), ref.pred.strip(), " ".join(ref.args[1:]).strip()
        ref_ext = "(subject) " + ref_arg1 + " (relation) " + ref_pred + " (object) " + ref_arg2
        ex_arg1, ex_pred, ex_arg2 = ex.args[
            0].strip(), ex.pred.strip(), " ".join(ex.args[1:]).strip()
        ex_ext = "(subject) " + ex_arg1 + " (relation) " + ex_pred + " (object) " + ex_arg2
        prec_bleu = sentence_bleu(references=[ref_ext.split(' ')],
                                  hypothesis=ex_ext.split(' '))
        recall_bleu = sentence_bleu(references=[ex_ext.split(' ')],
                                    hypothesis=ref_ext.split(' '))
    ...
    def binary_linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        if len(ex.args) >= 2:
            ...
            e = ex
        stright_match = Matcher.linient_tuple_match(r, e, ignoreStopwords,
                                                    ignoreCase)
        ...
        if not said_type_sentence:
            ...
            if len(ex.args) >= 2:
                ...
                e = ex
            reverse_match = Matcher.linient_tuple_match(r, e, ignoreStopwords,
                                                        ignoreCase)
