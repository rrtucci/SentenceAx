...
class AllennlpReader(OieReader):
    ...
    def read(self, fn):
        ...
        for line in fin:
            ...
            # curExtraction = Extraction_sax(pred = rel, head_pred_index = -1, sent = text, confidence = -1/float(confidence))
            # curExtraction = Extraction_sax(pred = rel, head_pred_index = -1, sent = text, confidence = math.exp(float(confidence)))
